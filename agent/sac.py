...
class SAC(object):
    ...
    def update_actor_and_alpha(self, obs, logger, step, bc_obs=None, bc_action=None, log_this_step=True):
        ...
        bc_loss = None
        if bc_obs is not None and bc_action is not None:
            actor_mean = self.actor(bc_obs).mean
    ...
    def cqlV(self, obs, network, num_random=10, temp=None):
        """For CQL style training."""
        # importance sampled version
        if temp is None:
            temp = self.alpha.detach()
        elif not torch.is_tensor(temp):
            temp = torch.tensor(temp, device=obs.device, dtype=obs.dtype)
        ...
        cat_Q = torch.cat(
            [rand_Q - temp * random_density, current_Q - temp * log_prob.detach()], 1
        )
        cql_V = torch.logsumexp(cat_Q / temp, dim=1).mean() * temp
        return cql_V
