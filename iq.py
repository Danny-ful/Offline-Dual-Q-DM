...
def _compute_dynamics_penalty(agent, batch, *, shared_noise=True):
    ...
    with torch.no_grad():
        ...
        next_states = ens.sample_next_ensemble(
            obs, action, M=M, noise=dynamics_noise)
...
def synthetic_iq_loss(agent, obs, step, log_this_step=False):
    ...
    with torch.no_grad():
        ...
        flat_raw_s = (flat_s if normalizer is None
                      else normalizer.denormalize_tensor(flat_s))
